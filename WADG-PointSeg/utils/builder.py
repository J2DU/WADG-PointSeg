...
def get_stf(phase='test'):
    from datasets import SemanticSTF
    dataset_config = configs.src_dataset if phase == 'train' else configs.tgt_dataset
...
def make_model() -> nn.Module:
    if configs.model.name == 'WADGPointSegNet':
        from models.model import WADGPointSegNet
        if 'cr' in configs.model:
            cr = configs.model.cr
        else:
            cr = 1.0
        model = WADGPointSegNet(num_classes=configs.data.num_classes, cr=cr)
    else:
        raise NotImplementedError(f"Model {configs.model.name} is not implemented.")
    return model
